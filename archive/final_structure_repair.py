...
def fix_malformed_html(content):
    ...
    for i, line in enumerate(lines):
        ...
        # Pattern: <body class="calibre"><div class="contentsh"><a href="..." class="...">...</a></li>
        if '<body class="calibre"><div class="contentsh">' in line and '</li>' in line and '<a href=' in line:
            # Extract the href and text content
            match = re.search(r'<a href="([^"]+)" class="([^"]+)">([^<]+)</a></li>', line)
            if match:
                href, a_class, text = match.groups()
                # Replace the entire malformed structure
                line = re.sub(
                    r'<body class="calibre"><div class="contentsh"><a href="[^"]+" class="[^"]+">[^<]+</a></li>',
                    f'<body class="calibre"><div class="contentsh"><a href="{href}" class="{a_class}">{text}</a></div>',
                    line
                )
                print(f"Fixed malformed body/div structure in line {i+1}")
        
        # Fix malformed h1 patterns like "h1 iii" -> "<h1>iii</h1>"
        if re.search(r'\bh1\s+\w+', line) and '<h1' not in line:
            line = re.sub(r'\bh1\s+(\w+)', r'<h1>\1</h1>', line)
            print(f"Fixed malformed h1 in line {i+1}")
        
        # Fix broken div tags that don't have proper closing
        if '<div' in line and not line.count('<div') == line.count('</div>'):
            div_opens = line.count('<div')
            div_closes = line.count('</div>')
            missing_closes = div_opens - div_closes
            
            if missing_closes > 0:
                # Add missing closing divs before </body> or at end of line
        ...
        # Fix broken h1 tags that don't have proper closing
        if '<h1' in line and not line.count('<h1') == line.count('</h1>'):
            h1_opens = line.count('<h1')
            h1_closes = line.count('</h1>')
            missing_closes = h1_opens - h1_closes
            
            if missing_closes > 0:
                # Add missing closing h1s before </body> or at end of line
        ...
        if line != original_line:
            print(f"  Original: {repr(original_line[:100])}")
            print(f"  Fixed:    {repr(line[:100])}")
