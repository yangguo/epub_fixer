...
def fix_xml_declaration(content):
    """Fix malformed XML declarations"""
    # Fix XML version declaration
    content = re.sub(r"version='1\.0='", "version='1.0'", content)
    content = re.sub(r'version="1\.0="', 'version="1.0"', content)
    ...
    # Fix any other malformed attributes in XML declaration
    content = re.sub(r"(<\?xml[^>]*?)='([^']*?)='", r"\1='\2'", content)
    content = re.sub(r'(<\?xml[^>]*?)="([^"]*?)="', r'\1="\2"', content)
    
    return content

def process_html_files(extract_dir):
    """Process all HTML files to fix XML declarations"""
    html_files = []
    ...
    for file_path in html_files:
        try:
            ...
            fixed_content = fix_xml_declaration(content)
    ...
    print(f"Fixed XML declarations in {fixed_files} files")
    return fixed_files
