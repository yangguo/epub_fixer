...
def fix_missing_equals(content):
    """Fix missing = characters in HTML attributes"""
    # Fix patterns like: attribute"value" -> attribute="value"
    # This handles cases where the = was removed between attribute name and quoted value
    content = re.sub(r'(\w+)"([^"]*?)"', r'\1="\2"', content)
    
    # Fix patterns like: attribute'value' -> attribute='value'
    content = re.sub(r"(\w+)'([^']*?)'", r"\1='\2'", content)
    
    # Fix specific cases that might have been corrupted
...
def process_html_files(extract_dir):
    """Process all HTML files to fix missing equals in attributes"""
    html_files = []
    ...
    for file_path in html_files:
        try:
            ...
            fixed_content = fix_missing_equals(content)
...
def main():
    ...
    # Fix missing equals in attributes
