...
def fix_div_closure(content):
    """Fix missing closing div tags and malformed h1 tags"""
    
    # Fix malformed h1 tags (like h1 iii or h1 acknowledgment)
    content = re.sub(r'<h1\s+([a-zA-Z]+)\s*(?![>=])', r'<h1 class="\1">', content)
    
    # Count opening and closing div tags more accurately
    # Find all opening div tags
    open_divs = re.findall(r'<div[^>]*>', content)
    close_divs = re.findall(r'</div>', content)
    
    open_count = len(open_divs)
    close_count = len(close_divs)
    
    print(f"Found {open_count} opening divs and {close_count} closing divs")
    ...
    if missing_closes > 0:
        ...
        if body_end_match:
            ...
            if html_end_match:
                ...
                # If no </html> tag, add at the very end
                closing_divs = '\n'.join(['</div>'] * missing_closes)
                content += '\n' + closing_divs
...
def process_epub(epub_path):
    """Process EPUB file to fix div closure issues"""
    
    # Create backup
    backup_path = epub_path.replace('.epub', '_backup_div.epub')
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for html_file in html_files:
            try:
                ...
                fixed_content = fix_div_closure(content)
                ...
                if fixed_content != original_content:
                    ...
                    print(f"No changes needed for: {os.path.basename(html_file)}")
...
def main():
    ...
    print("Starting div closure fix...")
