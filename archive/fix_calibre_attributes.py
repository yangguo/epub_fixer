...
def fix_calibre_attributes(content):
    """Fix specific calibre attribute patterns"""
    # Fix patterns like: calibre"> to class="calibre">
    content = re.sub(r'\s+calibre">', r' class="calibre">', content)
    
    # Fix patterns like: calibre1"> to class="calibre1">
    content = re.sub(r'\s+calibre(\d+)">', r' class="calibre\1">', content)
    
    # Fix patterns like: calibre > to class="calibre" >
    content = re.sub(r'\s+calibre\s*>', r' class="calibre">', content)
    
    # Fix patterns like: calibre1 > to class="calibre1" >
    content = re.sub(r'\s+calibre(\d+)\s*>', r' class="calibre\1">', content)
    
    # Fix patterns like: calibre followed by space and other attributes
    content = re.sub(r'\s+calibre\s+', r' class="calibre" ', content)
    content = re.sub(r'\s+calibre(\d+)\s+', r' class="calibre\1" ', content)
    
    # Fix patterns like: eula"> to class="eula">
    content = re.sub(r'\s+eula">', r' class="eula">', content)
    
    # Fix patterns like: eula > to class="eula" >
    content = re.sub(r'\s+eula\s*>', r' class="eula">', content)
    
    # Fix patterns like: eula followed by space
    content = re.sub(r'\s+eula\s+', r' class="eula" ', content)
    
    return content

def fix_meta_charset(content):
    """Fix malformed meta charset attributes"""
    # Fix patterns like: charset="UTF-8" to charset="utf-8"
    content = re.sub(r'charset="UTF-8"', r'charset="utf-8"', content)
    
    # Fix patterns where UTF-8 appears as an attribute name
    content = re.sub(r'\s+UTF-8="[^"]*"', r'', content)
    
    return content

def process_html_files(extract_dir):
    """Process all HTML files to fix calibre attributes"""
    html_files = []
    ...
    for file_path in html_files:
        try:
            ...
            # Fix calibre attributes
            content = fix_calibre_attributes(content)
            
            # Fix meta charset issues
