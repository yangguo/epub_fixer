...
def fix_malformed_attributes(content):
    """Fix all malformed attributes in HTML content"""
    # Fix xmlns namespace with extra =
    content = re.sub(r'xmlns="([^"]*?)="', r'xmlns="\1"', content)
    content = re.sub(r"xmlns='([^']*?)='", r"xmlns='\1'", content)
    
    # Fix http-equiv with extra =
    content = re.sub(r'http-equiv="([^"]*?)="', r'http-equiv="\1"', content)
    content = re.sub(r"http-equiv='([^']*?)='", r"http-equiv='\1'", content)
    
    # Fix content attribute with extra =
    content = re.sub(r'content="([^"]*?)="', r'content="\1"', content)
    content = re.sub(r"content='([^']*?)='", r"content='\1'", content)
    
    # Fix charset with extra =
    ...
    # Fix rel with extra =
    content = re.sub(r'rel="([^"]*?)="', r'rel="\1"', content)
    content = re.sub(r"rel='([^']*?)='", r"rel='\1'", content)
    
    # Fix class with extra =
    ...
    # Fix attributes that are missing values entirely (like calibre, calibre1, etc.)
    # Pattern: attribute_name followed by space or > without = and value
    content = re.sub(r'\s+(calibre\d*)\s*>', r' class="\1">', content)
    content = re.sub(r'\s+(calibre\d*)\s+', r' class="\1" ', content)
    
    # Fix any remaining malformed attributes with pattern: name="value="
    ...
    for file_path in html_files:
        try:
            ...
            if fixed_content != original_content:
                ...
                fixed_files += 1
                print(f"Fixed malformed attributes in {os.path.basename(file_path)}")
                
        except Exception as e:
            print(f"Error processing {file_path}: {e}")
    
    print(f"Fixed malformed attributes in {fixed_files} files")
    return fixed_files
